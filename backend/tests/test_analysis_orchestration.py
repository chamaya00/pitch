...
class Fixture:
    ...
    def temp_files(self) -> list[str]:
        directory = self.analyses.directory
        if not directory.is_dir():
            return []
        return [path.name for path in directory.iterdir() if path.name.startswith("analysis-")]
    ...
    def seed_analysis(self, **overrides: object) -> Analysis:
        ...
        return self.analyses.create(Analysis.model_validate(payload))
...
def env(tmp_path: Path) -> Fixture:
    ...
    recordings = JsonFileRecordingRepository(tmp_path)
    recordings.create(
        Recording(
    ...
    return Fixture(
        ...
        analyses=JsonFileAnalysisRepository(tmp_path),
        recording_id=stored.recording_id,
    )
...
def analyze(service: AnalysisService, recording_id: str) -> Analysis:
    return run(lambda: service.analyze(recording_id))
...
def test_the_analysis_is_persisted_and_readable_afterwards(env: Fixture):
    analysis = analyze(env.service(), env.recording_id)
    stored = env.analyses.get(analysis.analysis_id)

    assert stored == analysis
    assert (env.analyses.directory / f"{analysis.analysis_id}.json").is_file()
...
def test_the_record_is_written_at_every_stage(env: Fixture):
    ...
    def observe(analysis: Analysis) -> Analysis:
        seen.append(analysis.status)
        return original_update(analysis)
...
def test_no_temporary_file_remains_after_a_successful_analysis(env: Fixture):
    analysis = analyze(env.service(), env.recording_id)

    assert env.temp_files() == []
    assert sorted(p.name for p in env.analyses.directory.iterdir()) == [
...
def test_the_recording_is_untouched_by_a_failed_analysis(env: Fixture):
    ...
    stored_recording = env.recordings.get(env.recording_id)
    ...
    assert env.recordings.get(env.recording_id) == stored_recording
...
def test_an_unknown_recording_creates_no_analysis_record(env: Fixture):
    ...
    assert not env.analyses.directory.exists() or list(env.analyses.directory.iterdir()) == []
...
def test_concurrent_starts_converge_on_one_analysis(env: Fixture):
    ...
    Note what this does and does not show. With the synchronous filesystem
    store the find-or-create section never yields, so it would pass even
    without the lock in ``analysis.py`` — the lock is there for the async store
    that replaces it. What this pins down is the observable behaviour a caller
...
def test_a_retry_leaves_the_failed_analysis_inspectable(env: Fixture):
    ...
    preserved = env.analyses.get(failed.analysis_id)
    assert preserved == failed
    assert preserved is not None
    assert preserved.error_code is ErrorCode.ANALYSIS_PROVIDER_TIMEOUT
    assert len(env.analyses.list_for_recording(env.recording_id)) == 2
...
def test_an_abandoned_analysis_is_recorded_as_failed_rather_than_left_running(env: Fixture):
    ...
    swept = env.analyses.get(abandoned.analysis_id)
...
def test_cancellation_leaves_a_record_and_propagates(env: Fixture):
    ...
    stored = env.analyses.get(pending.analysis_id)
    assert stored is not None
...
def test_a_store_that_breaks_while_recording_a_failure_reports_the_real_cause(env: Fixture):
    ...
    def fail_only_on_the_failure_write(analysis: Analysis) -> Analysis:
...
def test_a_completed_analysis_always_carries_its_measurements(env: Fixture):
    for feedback in (MockFeedbackProvider(), FailingFeedback(ProviderResponseError("claude"))):
        env.analyses.directory.mkdir(parents=True, exist_ok=True)
        for path in env.analyses.directory.iterdir():
            path.unlink()
