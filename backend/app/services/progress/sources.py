...
**The eleventh went in Step 10.18, and what demanded it was the window rather
than the data.** This query is flat in the size of the database — 28.9 ms at 50
recordings and 28.0 ms at 5 000, because the index locates the window — but
linear in the window itself, which is a documented parameter that goes to 200.
At 200 it was 141 ms, all of it decompressing a 253 kB document per row to read
~600 bytes of metrics out of it. Migration 0006 makes that projection a stored
generated column, so this statement no longer mentions ``document``:

==========================================  =========  =========
approach                                    time       TOAST reads
==========================================  =========  =========
``document->'metrics'``, window of 200       141.1 ms      7 127
the ``metrics`` column, window of 200          1.7 ms        262
==========================================  =========  =========

Generated rather than written by the repository, so there is no rule to keep:
PostgreSQL computes it from the document in the same write, and a row whose
metrics disagree with its document cannot be produced.
...
#:
#: ``a.metrics`` is a **column** (migration 0006), generated by PostgreSQL as
#: ``document -> 'metrics'``. Until Step 10.18 it was that expression written
#: here, which detoasted the whole 253 kB document once per row to read ~600
#: bytes out of it. The column is the same projection, computed once at write
#: time by the server rather than on every read, so this statement no longer
#: references ``document`` at all and no row of a window is decompressed.
#:
#: It is ``NULL`` for a recording with no completed analysis, exactly as
#: ``a.document`` and the expression before it were, so every scalar below stays
#: ``NULL`` in that case and nothing downstream changes.
PROGRESS_SQL = """
SELECT * FROM (
    SELECT r.id                AS recording_id,
      ...
      FROM recordings r
      LEFT JOIN LATERAL (
          SELECT status, metrics FROM audio_analyses
